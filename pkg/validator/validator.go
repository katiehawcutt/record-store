...
func ValidateAlbum(album a.AlbumInput) error {
	return v.ValidateStruct(&album,
		v.Field(&album.Title, v.Required, v.Length(1, 50).Error("The title must be between 1-50 digits")),
		v.Field(&album.Artist, v.Required, v.Length(1, 50).Error("The artist must be between 1-50 digits")),
